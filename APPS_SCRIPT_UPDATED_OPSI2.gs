 ...
 * ✅ UPDATED: Dengan Role System (OPSI 2 - AMAN)
 *
 * DEPLOYMENT SETTINGS:
 * - Execute as: Me
 ...
 *
 * OPSI 2 (AMAN): userRole di akhir - TIDAK menggeser kolom existing
 ...
 * N:proteksi | O:google_review | P:mgb | Q:photos | R:notes | S:userRole
 *
 * PERUBAHAN DARI VERSI LAMA:
 * - Kolom S (userRole) BARU ditambahkan di akhir
 * - Kolom A-R TIDAK BERUBAH (aman untuk data existing)
 */
...
// Urutan indikator (sesuai urutan kolom H-P di spreadsheet - TETAP SAMA)
...
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: 'ok',
      message: 'Crown Daily Indicators API is running! (with Role System - Opsi 2)',
...
// ─── addSubmission ────────────────────────────────────────────────────────────
/**
 * ✅ UPDATED: Menyimpan submission DENGAN ROLE (OPSI 2 - AMAN)
 *
 * Kolom H–P = nilai tiap indikator (TETAP SAMA)
 * Kolom Q   = foto (TETAP SAMA)
 * Kolom R   = notes (TETAP SAMA)
 * Kolom S   = userRole (BARU - di akhir)
 *
 * PERUBAHAN:
 * - Tambah kolom S: userRole (Advisor/Cashier/CS) di AKHIR
 * - Kolom A-R TIDAK BERUBAH
 */
function addSubmission(submission) {
  try {
    ...
    // Row: A–G = metadata, H–P = indikator, Q = foto, R = notes, S = userRole
    var rowData = [
      submission.id || '',                                          // A: id
      ...
      photosStr,                                                    // Q: photos
      notesStr,                                                     // R: notes
      userRole                                                      // S: userRole ✅ BARU - DI AKHIR!
    ];
...
function updateSubmissionsHeader() {
  ...
  // Header baru dengan userRole di kolom S (akhir)
  var newHeaders = [
    'id',           // A
    'branchId',     // B
    'userNik',      // C
